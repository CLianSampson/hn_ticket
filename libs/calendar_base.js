...
(function() {
	...
	Calendar.prototype = {
		...
		initEntry: function(op) {
			...
			self.refreshData({
				...
				self.refreshData({
					...
					var html = "";
					//此处outputs为每一天对应的html  add by chenlian on 2018-07-25
		...
		refreshData: function(dateObj, callback) {
			...
			//星期几为每个月的第一天 changed by sampson chen on 2018-07-25
			...
			for(var i = 1; i < nowXingQiJi + 1; i++) {
				//此处if应改在外面，但在外面则代码不易观察   changed by sampson chen on 2018-07-25
				if (nowXingQiJi==7) {
					break;
				}
			...
			var daonale = 0; //到哪了拼音,增加注释  sampson chen on2018-07-25
			...
			for(var index2 = daonale + 1, index3 = 1; index2 <= 42; index2++, index3++) {
				//去掉尾部, changed by sampson chen on 2018-07-25
				if (index2==36) {
					break;
				}
			...
			for(var i = 0, len = dataObj.tmpInfo.length; i < len; i++) {
				...
				if(!templData) {
					...
					if(value.date == curr) {
						//今天
						if(value.isSelected) {
							//changed by sampson on 2017-07-25  去掉em-calendar-active  增加  em-calendar-today
							template = '<div class="em-calendar-item em-calendar-today   isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p><span class="dot dot-type1"></span></div>';
//							template = '<div class="em-calendar-item em-calendar-active  isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p><span class="dot dot-type1"></span></div>';
							...
							//changed by sampson on 2017-07-25  去掉em-calendar-active
							template = '<div class="em-calendar-item em-calendar-today   isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p></div>';
//							template = '<div class="em-calendar-item em-calendar-active  isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p></div>';
						...
						var  str = self.getLunarMonthSpecial(value);
						if (str!='') {
							template = str;
						}
		 ...
		 * 使农历月份显示红色
		 * @param {Object} value
		 * 此处不能使用循环，用循环会卡死  add by sampson chen on 2018-07-26 
		 */
		getLunarMonthSpecial: function(value){
			var template = '';
			if (value.lunar == '正月') {
				template = '<div class="em-calendar-item em-calendar-lunar-month isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">{{lunar}}</p><span class="dot dot-type1"></span></div>';
			}
			if (value.lunar == '二月') {
				template = '<div class="em-calendar-item em-calendar-lunar-month isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">{{lunar}}</p><span class="dot dot-type1"></span></div>';
			}
				...
				onClick: function(swiper, e) {
					...
					if(!_this.classList.contains('isforbid0')) {
						if(_this.getAttribute("date")) {
							...
							//如果是今天被点击，则取消今天的样式  changed by samposn on 2018-07-25
							var curr = self.currentDate;
							if (curr==dateStr) {
								if (_this.classList.contains('em-calendar-today')) {
									_this.classList.remove('em-calendar-today');
								}else{
									_this.classList.add('em-calendar-today');
								}
							}
							
							if (!self.compareWith30Days(dateStr)) {
								alert("请选择30天以内的日期");
								return;
							}
							
							if (!self.compareWithCurrentTime(dateStr)) {
								alert('请选择当天或者当天以后的日期');
								return;
							}
							
							if (_this.classList.contains('em-calendar-active')) {
								_this.classList.remove('em-calendar-active');
								globalDateArry.pop(dateStr);
							}else{
								_this.classList.add('em-calendar-active');
								globalDateArry.push(dateStr);
							}
...
//							});
				...
				onSlideNextStart: function(swiper) {
					...
					//去掉渲染1号样式 changed by sampson chen on 2018-07-25
...
//			tmp += "月";
			...
			//将初一换成月份 changed by chenlian on 2018-07-25
			if (tmp == '初一') {
				tmp = self._getMonthByNumber(self.cMonth);
		...
		_getLunarDay: function(solarYear, solarMonth, solarDay) {
			...
			if(solarYear < 1921 || solarYear > 2080) {
				...
				console.log('lunar is :');
				console.log(self._getcDateString());
		...
		/**
		 * 将数字转换成对应文字  add by sampson chen on 2018-07-25
		 * @param {Object} number  
		 */
		_getMonthByNumber: function(number){
			//有了return不需要break
			switch (number){
				case 1:
					return '正月';
		 ...
		 */
		compareWithCurrentTime: function(timeStr){
		    ...
		    var currentDate = new Date(dayStr);
		    var compareDate = new Date(timeStr);
  		...
  		//将传入字符串与当前时间对比， 参数格式“yy-MM-dd”
	  	compareWith30Days: function(timeStr){
		    //30天后的时间
		    var nowDate = new Date();
